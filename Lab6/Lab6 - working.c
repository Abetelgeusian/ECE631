...
int main(int argc, char *argv[]){
    ...
    // since there are whitespaces in the file, we will write out another file without
    // these white spaces.
    data_pts = 0;
    // fpt1=fopen("data_noheader.txt","w"); // data is x_acc y_acc z_acc pitch roll yaw not rpy
    while(fscanf(fpt,"%lf %lf %lf %lf %lf %lf %lf ",&t[data_pts],&acc_x[data_pts],&acc_y[data_pts],&acc_z[data_pts],&pitch[data_pts],&roll[data_pts],&yaw[data_pts]) != EOF){
        // fprintf(fpt1,"%lf %lf %lf %lf %lf %lf %lf\n",t[data_pts],acc_x[data_pts],acc_y[data_pts],acc_z[data_pts],pitch[data_pts],roll[data_pts],yaw[data_pts]);
        data_pts++;
    }
    // while(fscanf(fpt,"%lf %lf %lf %lf %lf %lf %lf ",&t[data_pts],&acc_x[data_pts],&acc_y[data_pts],&acc_z[data_pts],&roll[data_pts],&pitch[data_pts],&yaw[data_pts]) == 7)
    // {
    //     fprintf(fpt1,"%lf %lf %lf %lf %lf %lf %lf\n",t[data_pts],acc_x[data_pts],acc_y[data_pts],acc_z[data_pts],roll[data_pts],pitch[data_pts],yaw[data_pts]);
    //     data_pts++;
    // } // This loop also works. Changing the size from 640 to 1000 did the trick.
    fclose(fpt);
    // fclose(fpt1);

    // Let us smooth out the data. Let us use the simplest filter - mean filter.
    filt_win = 15;
    // First the edge cases. Since out filter has window, there will be some data points that
    // will not be smoothed. Let us take care of them first.
    for(x=0;x<filt_win/2;x++){
        f_acc_x[x] = acc_x[x];
        f_acc_y[x] = acc_y[x];
        f_acc_z[x] = acc_z[x];
        f_pitch[x] = pitch[x];
    ...
    for(x=data_pts-(filt_win/2);x<data_pts;x++){
        f_acc_x[x] = acc_x[x];
        f_acc_y[x] = acc_y[x];
        f_acc_z[x] = acc_z[x];
        f_pitch[x] = pitch[x];
    ...
    for (x =filt_win/2; x<(data_pts-(filt_win/2)); x++){
        sum_x = 0; sum_y =0; sum_z=0;
        sum_p = 0; sum_r = 0; sum_yaw=0;
        for(y=(-filt_win/2);y<=(filt_win/2);y++)
        {
        ...
        f_acc_x[x] = sum_x/filt_win;
        f_acc_y[x] = sum_y/filt_win;
        f_acc_z[x] = sum_z/filt_win;
        f_pitch[x] = sum_p/filt_win;
        f_roll[x] = sum_r/filt_win;
        f_yaw[x] = sum_yaw/filt_win;
    }
