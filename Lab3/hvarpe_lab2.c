...
for(T=0;T<=255;T++){
    ...
    // for(i=0;i<ROWS;i++){
    //     for(j=0;j<(COLS);j++){
    //         if (img_msf_norm[i*COLS+j]>=T){binary[i*COLS+j]=255;}
    //         else{binary[i*COLS+j]=0;}
    //     }
    // }
    ...
    while(fscanf(g_truth,"%s %d %d",gt_letter,&gt_col,&gt_row) != EOF){
        ...
        if (detected == 1){
            ...
            // }
            // for(i=0;i<=8;i++){
            // printf("%d",neighbor[i]);} // Eveyrthing works till here.
            ...
            while(del_counter > 0){
                ...
                for(r=0;r<15;r++){
                    for(c=0;c<9;c++){
                        ...
                        { // neighbor check
                            ...
                            /* test code*/
                            //    if (r == 13 && c == 1){del_counter = del_counter - 1;printf("Test complete!");}
                            //    else{printf("Test going on!");} 
                        } // if 
                    } // for
                }//for loop
                //  delete marked cells
                ...
                // for(i=0;i<(temp_C*temp_R);i++){if(thin_img[i] == 255){copy[i]=0;}}
                //  if(*gt_letter == letter){
                //     fpt = fopen("thresholded.ppm","wb");
                //     fprintf(fpt,"P5 %d %d 255\n",temp_C,temp_R);
                //     fwrite(copy,1,temp_R*temp_C,fpt);
...
// fpt = fopen("output.ppm","w");
// fprintf(fpt,"P5 %d %d 255 \n",COLS,ROWS);
// fwrite(img_msf_norm,COLS*ROWS,1,fpt);
// fclose(fpt);
}
