...
for(i=0;i<temp_R;i++){
    for(j=0;j<(temp_C);j++){
        mean += (float)template_img[i*temp_R+j];
    }
}
...
for(i=0;i<temp_R;i++){
for(j=0;j<(temp_C);j++){
    temp_img[i*temp_C+j] = (int)((float)template_img[i*temp_C+j] - mean);
}
} 
...
for(i=0;i<ROWS;i++){
    for(j=0;j<ROWS*COLS;j++){
        if(img_msf[i*COLS+j]>max){max = img_msf[i*COLS+j];}
        else if(img_msf[i*COLS+j]<min){min = img_msf[i*COLS+j];}
    }
}
// printf("%d is min \n",min);
// printf("%d is max \n",max);
